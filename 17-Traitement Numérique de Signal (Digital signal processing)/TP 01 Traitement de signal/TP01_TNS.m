...
f = (-n/2:(n/2)-1)*(fe/n);     % frequency range
power = fftshift(abs(Xf).^2/n);
figure(7)
plot(f,abs(power));
grid on 
...
xlabel('Freaunces')
ylabel('Amplitude(')
title('DSP de Signal échantillonné par une fréquence de 100.2Hz')
grid on 
...
ylabel('Amplitude(u.a)')
title('La DSP du Signal échantionné par une fréquence de 500Hz')
grid on

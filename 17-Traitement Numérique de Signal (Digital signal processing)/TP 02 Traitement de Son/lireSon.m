...
%cette fonction permet dans le repertoire indiqu0ar rep de:
%  d¯mbrer le nombre de fichiers .mat disponibles pour lire le son
%  tirer au hasard un fichier .mat disponible et lire y et fe
%  lire y et fe du fichier .mat num¯ m. 
  switch(m)
    case -1
      rp1=nbFichiers(rep);
      rp2=' ';
...
function nom=c_nom(rep,m)
  %d%rmine le nom dans S du fichier m
  est_ok=@(nom)(length(nom)>4)&&(strcmp('.mat',nom(end-3:end)));
  S=dir(rep);

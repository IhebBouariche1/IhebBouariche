...
class Actor(nn.Module): 
  ...
  def forward(self,state):
    action = 2*self.actor(self.sharedforward(state))
    return action 
...
env = KukaDiverseObjectEnv(renders=False, isDiscrete=False, removeHeightHack=False,  actionRepeat=80, maxSteps = 20,numObjects = 1,blockRandom=0.2 )
...
#noise = torch.tensor([0,0,0]).to(device)
noise_std = torch.tensor([1,1,1]).to(device)
noise_decay_rate = torch.tensor([4e-5,4e-5,4e-5]).to(device) #15000 episodes for 100% of exploration 
...
Policy = DDPG(state_size,action_dim,Actor_learning_rate = 0.00001,Critic_learning_rate = 0.00001,discount_factor = 0.99,buffer_size = 1e6)
...
for e in range(max_episodes):
  ...
  for step in range(max_steps): 
    ...
    state = get_screen(env)
    noise_std, noise = Policy.Noise(noise_std,noise_decay_rate)
    
    if e >= 5000:
      states,actions,rewards,next_states,dones = Policy.step(env,state,noise)
    else: 
      states,actions,rewards,next_states,dones = Policy.step_noise(env,state,noise)
    ...
    Actor_Loss, Critic_Loss,actions_target,values_target,y,Values_critic,Values_critic1,Values_critic2,actions1,actions2 = Policy.update(batch_states, batch_actions.to(device),batch_rewards.to(device), batch_next_states.to(device),batch_dones)  
    ...
    #print("\n batch_actions: ",batch_actions,"\n rewards: ", batch_rewards,"\n dones: ",batch_dones,"\n actions_target: ",actions_target,"\n values_target: ",values_target,"\n y: ",y,"\n Values_critic1: ",Values_critic1,"\n Values_critic2: ",Values_critic2, "Actor Loss: ",Actor_Loss,"Critic Loss: ",Critic_Loss)
    ...
    if (e+1) % 1 == 0 and dones[-1] == 1:
      f = s - k
      print("number of steps: ",f,"episode: ",e+1, " reward: ",rewards[-1], " Actor Loss: ",AL.detach().tolist()/f, " Critic Loss: ",CL.detach().tolist()/f , "noise", noise)
      Actor_LOSS_List.append(AL.detach().tolist()/f)
      Critic_LOSS_List.append(CL.detach().tolist()/f)
    ...
    if (e+1)%saving_rate==0 and dones[-1] == 1:
      ...
      print("===========================================================================================================")
      average_reward.append(d/saving_rate)

      with open('Actor_Loss.txt', 'a') as file:
          for item in Actor_LOSS_List:
              file.write(f'{item}\n')


      with open('Critic_Loss.txt', 'a') as file:
          for item in Critic_LOSS_List:
              file.write(f'{item}\n')
      ...
      if (d/saving_rate) >= 0.70:
            PATH1 = "Actor.pt"
            PATH2 = "Critic.pt"
            # Save the model to the path
            torch.save(Policy.Actor.state_dict(), PATH1)
            torch.save(Policy.Critic.state_dict(), PATH2)
            
            print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Files are created $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
            
            if (d/saving_rate) >= 0.85:
              stop_training = True
      d = 0
    if dones[-1]:
      break
